...
robots_url <- paste0(url_base, "robots.txt")

# Petición GET forzando HTTP/1.1 y con User-Agent de navegador
respuesta_robots <- GET(
  robots_url,
  ...
  config(http_version = 2) # En curl, el valor 2 fuerza el uso de HTTP/1.1
)

# Imprimir el contenido de robots.txt
if (status_code(respuesta_robots) == 200) {
  cat(content(respuesta_robots, as = "text", encoding = "UTF-8"))
} else {
  print(paste("Error en la solicitud. Código:", status_code(respuesta_robots)))
}
```
...
# 1. Obtener y mostrar el archivo robots.txt completo
robots_texto <- get_robotstxt("https://www.accuweather.com")
cat(robots_texto)

# 2. Comprobar directamente si una ruta específica está permitida para un bot
paths_allowed(
  domain = "https://www.accuweather.com",
  path = "/es/co/bogota/107060/weather-forecast/107060",
  user_agent = "*"
)
```
